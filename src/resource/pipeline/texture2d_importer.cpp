	...
	namespace resource {
		namespace pipeline {
			...
			Texture2DContent* Texture2DImporterImpl::import(const String& filename, FileSystem* fileSystem) {
				...
				if (index == String::npos) throw std::runtime_error("Couldn't determine the file type!");
				...
				for (size_t i = 0; i < height; ++i) std::copy(rows[i], rows[i] + rowByteCount, txc->pixels.data() + i * rowByteCount);
