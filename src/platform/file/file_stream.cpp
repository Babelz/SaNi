...
namespace sani {
	namespace io {
		...
		namespace priv {
			...
			int64 FileStreamImpl::read(unsigned char* buffer, const int32 size) const {
			...
			uint32 FileStreamImpl::write(const unsigned char* data, const int32 size) const {
			...
			int64 FileStreamImpl::read(unsigned char* buffer, const int32 offset, const int32 size) const {
				throw std::logic_error("Not implemented");
			}

			void FileStreamImpl::seek(const SeekPosition seekpos, const long offset) const {
			...
			void FileStreamImpl::flush() const {
