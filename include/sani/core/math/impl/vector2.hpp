...
namespace sani {
	namespace math {
		...
		template <typename T>
		inline Vector2<T>::Vector2(const T& value) : x(value), y(value) {
		...
		template <typename T>
		inline const Vector2<T> Vector2<T>::operator*(const Vector2<T>& rhs) const {
			return Vector2<T>(x * rhs.x, y * rhs.y);
		}

		template <typename T>
		inline const Vector2<T> Vector2<T>::operator*(const T& rhs) const {
